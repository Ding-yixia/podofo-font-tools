...
//   output/
//     TrueType/
//       Arial/full.pdf, Arial/subset.pdf
//       TimesNewRoman/full.pdf, TimesNewRoman/subset.pdf
//       ...
//     OpenType_CFF/
//       NotoSansSC-Regular/full.pdf, NotoSansSC-Regular/subset.pdf
//       ...
//     Type1/
//       ...

#include <podofo/podofo.h>
#include <podofo/private/podofo_config_private.h>
...
public:
    ...
    static FontInfo AnalyzeFontFile(const string& filePath)
    {
        ...
        if (dot != string::npos)
        {
            ...
            if (lower == ".ttc")
            {
                // For TTC: try each face, get info from first successful
                unsigned fi = 0;
...
public:
    ...
    {
        // Create output directory by category
        ...
        // Generate full-embed version (DontSubset)
        ...
        // Generate subset-embed version (default = subset enabled)
...
private:
                                  ...
                                  PdfFontCreateFlags embedFlags)
    {
        ...
        // Try to load the font - prefer SearchFont for embedding support
        PdfFont* pdfFont = nullptr;
        string fontName = font.FontName.empty() ? GetFileName(font.FilePath) : font.FontName;

        // First, try SearchFont (enables embedding via m_cachedQueries)
        ...
        // Use the font itself to draw text
        painter.TextState.SetFont(*pdfFont, 12.0);

        // Only draw characters that the font supports (prevent crashes)
        double tx = margin;
        string line;
        ...
        // Draw font metadata using Standard14 font (always available)
        ...
        if (labelFont)
        {
            ...
            // PDF Spec classification reference
            ...
            // Show the specific embedding key used
        ...
        // Before save, explicit embed to ensure all fonts are embedded
    ...
    static string BuildSafeString(PdfFont& font, const string& text)
    {
        ...
        {
            char32_t cp = 0;
            // Simple UTF-8 decode (text is ASCII so this is fine for our test strings)
...
void PrintUsage(const char* prog)
{
    ...
    cout << "  --type <type>   Filter by type: truetype, opentype, type1, all" << endl;
    cout << "  --help          Show help" << endl;
    cout << endl;
    cout << "Output structure:" << endl;
    cout << "  <output_dir>/" << endl;
    cout << "    TrueType/       <fontname>/full.pdf  (full embedding)" << endl;
    cout << "    TrueType/       <fontname>/subset.pdf (subset embedding)" << endl;
    cout << "    OpenType_CFF/   <fontname>/..." << endl;
    cout << "    Type1/          <fontname>/..." << endl;
    cout << "    CIDFontType0_CFF/  ..." << endl;
    cout << "    CIDFontType2_TrueType/ ..." << endl;
    cout << endl;
}
...
int main(int argc, char* argv[])
{
    ...
    try
    {
        ...
        for (auto& f : fonts)
        {
            ...
            if (filterType == "truetype" && f.SpecCategory != PdfSpecFontCategory::TrueType)
                continue;
            if (filterType == "opentype" && f.SpecCategory != PdfSpecFontCategory::OpenTypeCFF)
                continue;
            if (filterType == "type1" && f.SpecCategory != PdfSpecFontCategory::Type1
                && f.SpecCategory != PdfSpecFontCategory::Type1CFF)
                continue;
